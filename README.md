...
All are public and ungated. The CPT path now defaults to the ConlangCrafter
corpus (see [ConlangCrafter CPT](#conlangcrafter-cpt-canonical-foreign-distribution-dataset));
the legacy FineMath dataset above remains accessible via `--dataset-id
HuggingFaceTB/finemath` for reproducing the v1–v5 legacy CPT records.
...
## ConlangCrafter CPT (canonical foreign-distribution dataset)
...
amplify the signal, this repo's CPT path (`--data-mode cpt`) defaults to a
**synthetic constructed language**: ~11M tokens of native prose generated by
Vertex AI Gemini 3.5 Flash against a [ConlangCrafter](https://arxiv.org/abs/2508.06094)
language spec, published as
[`TearedModels/conlangcrafter-cpt-bd412d52`](https://huggingface.co/datasets/TearedModels/conlangcrafter-cpt-bd412d52).

Why a synthetic conlang:

- **Guaranteed novel.** The language did not exist before the synthesis run,
  so Qwen cannot have seen it during pretraining.
- **Infinitely extensible.** Re-run the synthesis script with a larger
  `--target-tokens` to scale the corpus to any track budget.
- **Deterministic.** The corpus is a function of the ConlangCrafter language
  id (`bd412d52`) and the generator model, so anyone can reproduce.

Alternatives considered and not pursued (see findings doc for the full
comparison): **SumTablets** Sumerian cuneiform transliterations (gave a bigger
absolute Track 2 drop but fixed at 82,452 rows — runs out of fresh tokens at
Track 3 epoch counts); **Linear A** and other undeciphered scripts
(corpora are 3+ orders of magnitude too small, and without decipherment any
loss drop would be memorization rather than language learning).

### Generating the conlang corpus

The script picks one of the 64 ConlangCrafter language specs in
[`malper/ConlangCrafter`](https://huggingface.co/datasets/malper/ConlangCrafter)
and synthesizes prose in it via Vertex AI Gemini. A per-chunk quality gate
(lexicon-overlap minimum, English-word maximum, minimum length) rejects
drifted chunks; failed chunks are retried up to twice with new topic seeds.

```bash
# Smoke first (≈50k tokens, ~3 min) to verify quality.
uv run python scripts/synthesize_conlang_cpt.py --smoke

# One-shot generation. Default picks the longest DeepSeek-R1 spec from
# malper/ConlangCrafter and targets ~10M output tokens (≈ $5-20 on Flash).
...
The published corpus is already wired as the default for `--data-mode cpt`,
so the existing CPT shortcuts pick it up automatically:

```bash
./run.sh cpt-smoke          # SDPA/no-compile, 6s sanity check
./run.sh track2             # 5-min sprint (CPT/LoRA)
./run.sh cpt-track1         # 30-min Track 1 run

# Conlang-named aliases (identical to the above unless you override the env var).
./run.sh conlang-smoke
./run.sh conlang-track2
./run.sh conlang-track1

# Point at a different conlang corpus you generated yourself.
export CONLANG_DATASET_ID=<your-hf-user>/conlangcrafter-cpt-<language_id>
./run.sh conlang-track1

# Reproduce a legacy FineMath CPT record.
./run.sh cpt-track1 --dataset-id HuggingFaceTB/finemath --dataset-config finemath-4plus
```

The `--cpt-text-field` flag (default `text`) picks the row column the CPT
loader reads, so `--dataset-id` accepts any HF dataset with a string text
column.
...
ConlangCrafter CPT sweep (2026-05-27): the synthetic conlang delivers a
dramatically larger eval-loss drop than the legacy FineMath baseline.
...
- Track 1 defaults to UltraChat general chat SFT, rendered as Qwen ChatML
  with assistant-only labels; `--data-mode cpt` switches to packed all-token
  CPT on the ConlangCrafter synthetic conlang corpus (the new canonical CPT
  default, see [ConlangCrafter CPT](#conlangcrafter-cpt-canonical-foreign-distribution-dataset)).
  CPT mode accepts any HF dataset with a string text column via
  `--dataset-id` and `--cpt-text-field` (default `text`)
